console.log('这是content scriptsss!');
...
function initCustomPanel()
{
	...
	`;
	console.log(document.getElementById('ziji'))
...
{	
	...
	if(tag === 'father'){
		console.log('父页面收到来自 ' + (sender.tab ? "content-script(" + sender.tab.url + ")" : "popup或者background") + ' 的消息：', request);
	}else if(tag === 'child'){
		console.log('子页面收到来自 ' + (sender.tab ? "content-script(" + sender.tab.url + ")" : "popup或者background") + ' 的消息：', request);
	}


	if(request.type === 'getChildUrl') {
		document.getElementsByTagName('iframe')[0]&&sendResponse(document.getElementsByTagName('iframe')[0].getAttribute('src'))
	}
	else if(request.type === 'testMessage'){
...
function set_tag(){
	// 可添加其他判断条件
	if(document.getElementsByTagName('iframe') && document.getElementsByTagName('iframe').length){
		document.getElementsByTagName('body')[0].setAttribute("name", "father")
	}else{
		document.getElementsByTagName('body')[0].setAttribute("name", "child");
	}	
}